        ...
        <div className="news">
          <div className="news-content">
            ...
            {sortedPost.map((news,index) => {
              ...
              return (
                index < showMorePosts ? <div className='line' key={news.title}> 
                ...
                <button type='button' onClick={()=>deleteHandlerNews(news.id)} className='delete-btn-comment'>x</button>
                  ...
                  <div className='d-flex control-bluebtn'>
                    <button type='button' className='readon-btn me-3 typology-pagination bluebtn'><Link to={`${news.id}`}>READ ON</Link></button>
                    <button type='button' className='readon-btn button-invert  typology-pagination classedit'><HiOutlineBookmark className='icon' /><Link to={`Editingnews/${news.id}`}>EDIT BLOG</Link></button>
